...
public class HTMLDecoder {
    public static final HashMap charTable = new HashMap();
    ...
    public static String decode(String s) {
        ...
        while (curPos < maxPos) {
            ...
            if (c == '&' && curPos2 < maxPos) {
                int tmpPos = curPos2 + 1;
                char d = s.charAt(curPos2);
                if (d == '#') {
                    if (tmpPos < maxPos) {
                        int tmpPos2 = tmpPos + 1;
                        char d2 = s.charAt(tmpPos);
                        if (d2 == 'x' || d2 == 'X') {
                            if (tmpPos2 < maxPos) {
                                int tmpPos3 = tmpPos2 + 1;
                                if (isHexDigit(s.charAt(tmpPos2))) {
                                    while (true) {
                                        if (tmpPos3 >= maxPos) {
                                            break;
                                        }
                                        tmpPos3++;
                                        char d3 = s.charAt(tmpPos3);
                                        if (!isHexDigit(d3)) {
                                            if (d3 == ';') {
                                                String t = s.substring(curPos2 + 2, tmpPos3 - 1);
                                                try {
                                                    int i2 = Integer.parseInt(t, 16);
                                                    if (i2 >= 0 && i2 < 65536) {
                                                        c = (char) i2;
                                                        i = tmpPos3;
                                                    }
                            ...
                            while (true) {
                                ...
                                int tmpPos4 = tmpPos2 + 1;
                                char d4 = s.charAt(tmpPos2);
                                if (isDigit(d4)) {
                                    tmpPos2 = tmpPos4;
                                } else if (d4 == ';') {
                                    String t2 = s.substring(curPos2 + 1, tmpPos4 - 1);
                                    try {
                                        int i3 = Integer.parseInt(t2);
                                        if (i3 >= 0 && i3 < 65536) {
                                            c = (char) i3;
                                            i = tmpPos4;
                                        }
                    ...
                    while (true) {
                        ...
                        tmpPos++;
                        char d5 = s.charAt(tmpPos);
                        if (!isLetterOrDigit(d5)) {
                            if (d5 == ';') {
                                String t3 = s.substring(curPos2, tmpPos - 1);
                                Character ch = (Character) charTable.get(t3);
                                if (ch != null) {
                                    c = ch.charValue();
                                    i = tmpPos;
                                }
    ...
    public static String compact(String s) {
        ...
        while (curPos < maxPos) {
            ...
            char c = s.charAt(curPos);
            if (isWhitespace(c)) {
                while (curPos2 < maxPos && isWhitespace(s.charAt(curPos2))) {
                    curPos2++;
                }
                c = ' ';
            }
